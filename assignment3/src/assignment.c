...
{ 
    ...
    char *taskName = NULL,                // task nr. given by cli parameter
         *matrixFilePath = MATRIX,        // path to the file containing matrix A
    ...
    if(taskName == NULL){ // if no parameter provided - just show help
        ...
        task1(argc, argv, epsilon, matrixFilePath, vectorBFilePath, vectorBSize);
    } else if(!strcmp(taskName, "2")) {
        task2(argc, argv, epsilon, matrixFilePath, vectorBFilePath, vectorBSize);
